...
class FocalLoss(nn.Module):
    def __init__(self, cfg: DictConfig, alpha: Optional[List[float]|torch.Tensor] = None, num_classes: Optional[int] = None):
        """
        ...
        if self.task_type == 'multi-class' and alpha is not None and isinstance(alpha, (list, torch.Tensor)):
    ...
    def multi_class_focal_loss(self, inputs, targets):
        ...
        ce_loss = -targets_one_hot * torch.log(probs + 1e-8)  # Add small epsilon to avoid log(0)
        ...
        if self.alpha is not None:
            alpha_t = alpha.gather(0, torch.argmax(targets_one_hot, dim=1))
            ce_loss = alpha_t.unsqueeze(1) * ce_loss

        # Apply focal loss weight
        loss = focal_weight.unsqueeze(1) * ce_loss

        # Sum over classes first (most entries are zero due to one-hot),
        # then reduce over the batch dimension
